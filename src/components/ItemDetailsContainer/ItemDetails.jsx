    ...
    return (
        <div className="row mb-5">
            <div className="col-md-6 offset-md-3 text-center color_marron">
                <img src={"../images/" + item.imagen} className="img-fluid" width={300} alt={item.nombre} />
                <h1>{item.nombre}</h1>
                <h3>{item.destacado}</h3>
                <p><b>${item.precio}</b></p>
                <p>{item.descripcion}</p>
                {counter === 0 ? <ItemCount initial={1} stock={item.stock} onAdd={onAdd} /> : <Link to={"/cart"} className="btn fondo_rosa">Ir al carrito</Link>}
            </div>
        </div>
    )
}
